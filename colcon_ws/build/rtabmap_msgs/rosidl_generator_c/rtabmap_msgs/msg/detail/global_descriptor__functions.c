  ...
  rtabmap_msgs__msg__GlobalDescriptor__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    rtabmap_msgs__msg__GlobalDescriptor * data =
      (rtabmap_msgs__msg__GlobalDescriptor *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!rtabmap_msgs__msg__GlobalDescriptor__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          rtabmap_msgs__msg__GlobalDescriptor__fini(&data[i]);
        }
        free(data);
        return false;
      }
