  ...
  rtabmap_msgs__msg__SensorData__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    rtabmap_msgs__msg__SensorData * data =
      (rtabmap_msgs__msg__SensorData *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!rtabmap_msgs__msg__SensorData__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          rtabmap_msgs__msg__SensorData__fini(&data[i]);
        }
        free(data);
        return false;
      }
