  ...
  rtabmap_msgs__msg__LandmarkDetection__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    rtabmap_msgs__msg__LandmarkDetection * data =
      (rtabmap_msgs__msg__LandmarkDetection *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!rtabmap_msgs__msg__LandmarkDetection__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          rtabmap_msgs__msg__LandmarkDetection__fini(&data[i]);
        }
        free(data);
        return false;
      }
